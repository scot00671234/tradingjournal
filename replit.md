...
- **Production 404 Fix (Jan 6, 2025)**: Resolved persistent coinfeedly.com 404 errors by implementing a comprehensive Nixpacks override solution. The root issue was Nixpacks auto-detecting Node.js and overriding custom configurations. Final solution: (1) Created nixpacks.toml with node provider disabled, (2) Added both Procfile and .nixpacks/start.sh for redundancy, (3) Ensured startup script properly starts Node.js server first then Caddy reverse proxy. This approach bypasses all Nixpacks auto-detection and forces the dual-service startup pattern required for the full-stack application.
...
- **Production 404 Fix (Jan 6, 2025)**: Resolved coinfeedly.com 404 errors by creating proper nixpacks.toml configuration that uses the dual-service startup script. The issue was Nixpacks trying to run Caddy directly instead of using the start.sh script that properly starts both Node.js server and Caddy reverse proxy. Fixed Caddyfile to correctly reverse proxy to localhost:3000 where the Node.js app serves both API and static files.